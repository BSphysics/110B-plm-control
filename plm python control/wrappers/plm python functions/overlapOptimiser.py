...
# from datetime import datetime
# from findGlobalPhaseMinimum import find_global_phase_minimum
...
# #def find_global_phase_minimum(self, images_per_batch=10, roi_slice=(slice(ymin, ymax), slice(xmin, xmax)),
#  #                             background_width=64, avg_repeats=3, save_path=None):
#     find_global_phase_minimum(self, images_per_batch=10, roi_slice=(slice(ymin, ymax), slice(xmin, xmax)), background_width=64, avg_repeats=3, save_path=None)
...
def overlap_optimiser(self, plm, camera):
    ...
    zero_phase = find_global_phase_minimum(
        self, plm, camera,
    ...
    # Old code for fitting a cos squared function to find the phase minimum - didn't really work very well
    # for idx in range(0,10):
    #     print('\nGlobal Phase = ' + str(idx*10))
    #     plm.pause_ui()
    #     plm_phase_map = simple_beam_maker(self, 0.8, 1, -1*(cumulative_tilt_x +cumulative_tilt_x_zoom) , 1*(cumulative_tilt_y + cumulative_tilt_y_zoom) , idx*10)   
