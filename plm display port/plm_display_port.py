...
from plough_pol_analyser import plough_pol
from bigSpotPolAnalyser import big_spot_pol_analyser
from hardwareTriggerROIOffset import hardware_trigger_roi_offset
...
camera.ChunkSelector.SetValue("Framecounter")
camera.ChunkEnable.SetValue(True)
...
# fullpath = r'D:\PLM\plm python control\wrappers\Display port\bin\plmctrl.dll'
numHolograms = 24

relativePath = r'..\bin\plmctrl.dll'

# # Create PLMController instance
plm = PLMController(MAX_FRAMES, N, M, relativePath, 1920, 0)
...
#****************************************************
    ...
    def init_ui(self):
        ...
        self.plough_pol_flag = False
        self.big_spot_pol_flag = False
        ...
        # Analyse polarisation of plough spot pattern 
        self.plough_pol_button = QPushButton('Plough pol analysis', self)
        self.plough_pol_button.setStyleSheet("background-color: green; color: yellow;")
        self.plough_pol_button.clicked.connect(self.plough_pol_analyser)
        self.middle_layout.addWidget(self.plough_pol_button)

        # Analyse polarisation of big spot pattern 
        self.big_spot_pol_button = QPushButton('Big spot pol analysis', self)
        self.big_spot_pol_button.setStyleSheet("background-color: Blue; color: white;")
        self.big_spot_pol_button.clicked.connect(self.big_spot_pol)
        self.middle_layout.addWidget(self.big_spot_pol_button)
    ...
    def plough_pol_analyser(self):
        self.plough_pol_flag = True
        self.update_value()

    def big_spot_pol(self):
        self.big_spot_pol_flag = True
        self.update_value()
    ...
    def update_value(self):
        ...
        if self.polarisation_measurement_flag:
            ...
            fname , folder_name = pol_measure(self.camera, global_amplitudes, '_Rotating linear polariser_' , self.camera.ExposureTimeAbs.GetValue() , multiBeamFilePath=self.multibeam_file_path, poincareBeamFilePath = self.poincare_file_path)
            self.polarisation_measurement_flag = False
            filename = os.path.join(folder_name, 'GUI screenshot.png')
            self.save_gui_screenshot(filename)

        if self.plough_pol_flag: 
            plough_pol()
            self.plough_pol_flag = False

        if self.big_spot_pol_flag: 
            big_spot_pol_analyser()
            self.big_spot_pol_flag = False
        ...
        if self.poincare_beam_flag:
            ...
            plm_frame = np.empty((numHolograms, M, N), dtype=np.float32)
            np.copyto(plm_frame, plm_phase_map[None, :, :])
            plm.bitpack_and_insert_gpu(plm_frame, 1)
        ...
        if self.multibeam_seq_flag:
            ...
            self.camera.StopGrabbing()
            all_images = np.zeros((800, 90, 112), dtype=np.uint8)
        ...
        if self.amp_ramp_frame_flag:    
            ...
            # Here we setup the hardware triggered acquistion at 1.4 kHz (using PLM triggers) but remember that the Hardware triggered
            ...
            tick_freq = self.camera.GevTimestampTickFrequency.Value  # Hz
            last_timestamp = None
            # Store: [frame_idx, blockID, timestamp_ticks, delta_t_seconds]
            timing_log = np.zeros((800, 4), dtype=np.float64)
            ...
            while self.camera.IsGrabbing():
                ...
                if grabResult.GrabSucceeded():
                    ...
                    img = grabResult.Array
                    all_images[idx]=img
                    # Compute frame-to-frame time
                    if last_timestamp is not None:
                        dt = (timestamp - last_timestamp) / tick_freq
                    else:
                        dt = np.nan  # first frame has no previous

                    # Store everything
                    timing_log[idx, 0] = idx
                    timing_log[idx, 1] = frame_number
                    timing_log[idx, 2] = timestamp
                    timing_log[idx, 3] = dt
            ...
            timing_log_clean[np.isnan(timing_log_clean[:, 3]), 3] = 0.0

            np.savetxt(timing_file,timing_log_clean,delimiter=",",header=header,comments='',fmt="%.6e")
            # np.savetxt(timing_file, timing_log, delimiter=",", header=header, comments='')
        ...
        if self.bitpack_enabled:
            ...
            for i in range(1):
                ...
                plm_frame = np.empty((numHolograms, M, N), dtype=np.float32)
                np.copyto(plm_frame, plm_phase_map[None, :, :])
                print("plm frame size = "+ str(np.shape(plm_frame)))
        ...
        if self.beam_A_SP_scan_enabled or self.beam_B_SP_scan_enabled or self.beam_A_complex_field_measurement_enabled or self.beam_B_complex_field_measurement_enabled:
            ...
            if self.centroid_x is None or self.centroid_y is None:
                print("Warning: No beam centroid found. Please run free-streaming first to locate the beam.")
                self.switch_to_free_streaming()
                return
            ...
            print(f"sPix_phase shape: {sPix_phase.shape}")

            # Pad so that (n_slices + 24 init slices) is divisible by 24
            n_init_slices = 24
            n_slices = sPix_phase.shape[2]
            total = n_slices + n_init_slices
            remainder = total % 24
            if remainder != 0:
                pad = 24 - remainder
                sPix_phase = np.concatenate(
                    [sPix_phase, np.zeros((sPix_phase.shape[0], sPix_phase.shape[1], pad), dtype=sPix_phase.dtype)],
                    axis=2
                )
                print(f"Padded sPix_phase by {pad} slices → new shape: {sPix_phase.shape}")

            sPix_phase_init = super_pixel_set_init(phase_tilt, sPix_phase)
            print(f"sPix_phase_init shape: {sPix_phase_init.shape}")  # should now be divisible by 24

            SP_frames = super_pixel_frames(sPix_phase_init)
            # SP_frames = SP_frames.astype(np.float32)
            superPixelFrames = SP_frames
            print("SP_frames shape = "+ str(np.shape(SP_frames)))


            H, W = phase_tilt.shape
            # numHolograms = 24
            # plm_frame = np.empty((numHolograms, H, W), dtype=np.float32)

            for i in range(SP_frames.shape[0]):
                print('\n Bitpacking super Pixel frame ' + str(i+1))
                plm_frame = SP_frames[i , :, :, :]
                plm_frame = np.ascontiguousarray(np.transpose(plm_frame, (2, 0, 1)), dtype=np.float32)
                # print("plm_frame shape = "+ str(np.shape(plm_frame)))
                plm.bitpack_and_insert_gpu(plm_frame, i)

            plm.resume_ui()
            plm.play()
            plm.play()
            sequence = np.arange(MAX_FRAMES, dtype=np.uint8)
            ...
            print(f"Using centroid: x={self.centroid_x}, y={self.centroid_y}")
            offset_x, offset_y = hardware_trigger_roi_offset(self.centroid_x, self.centroid_y,
            ...
            if self.beam_A_SP_scan_enabled:
                ...
                filename = os.path.join(saveFolder, 'superpixelframes.npy') 
                # np.save(filename , superPixelFrames[:5])
