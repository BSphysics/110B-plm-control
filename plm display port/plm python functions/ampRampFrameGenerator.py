...
# Aim of the function is to generate 24 plm phase maps. Each phase map will use a different combination of 
...
def amp_ramp_frame_generator(beamA_phase, beamB_phase, beamA_HG_amplitude, beamB_HG_amplitude):
    ...
    numHolograms = 24
    amp_ramp_scan_frame = np.empty((numHolograms, H, W), dtype=np.float32)  # numHolograms first

    for idx in range(len(amp_ramp)):
        global_amplitudes = np.array([amp_ramp[idx, 0], amp_ramp[idx, 1]])
        global_amplitudes /= np.max(global_amplitudes)
        ...
        # Assign to first dimension = hologram/frame
        amp_ramp_scan_frame[idx, :, :] = plm_phase_map.astype(np.float32)

    # Optional: ensure Fortran contiguous order if needed by PLM driver
    # amp_ramp_scan_frame = np.asfortranarray(amp_ramp_scan_frame)
